  ...
  const pluginOptions = Object.create(Object.prototype, {
      ...
      value: function getSrcDirFromProcess() {
        ...
        if (foundBabel > -1 && !args[foundBabel + 1].match("--")) {
          if (extname(args[foundBabel + 1]) === ".js") {
            ...
            return (
              relative(process.cwd(), args[foundBabel + 1])
            )
              ? relative(process.cwd(), args[foundBabel + 1])
              : "./";
          }
    ...
    defaultAlias: {
      ...
      value: function defaultAlias({ cssFileRelativeFromRoot, root, outDir, srcDir }) {
        ...
        if (cssFileRelativeFromRoot.slice(0, srcDir.length) === srcDir &&
          existsSync(resolve(root, join(outDir, cssFileRelativeFromRoot.slice(srcDir.length))))) {
      ...
      value: function requireCssFile(currentFile, value, root) {
        ...
        if (pluginOptions.thisPluginOptions.alias) {
          cssFileRelativeFromRoot = pluginOptions.thisPluginOptions.alias({
            cssFileRelativeFromRoot, root, outDir, srcDir,
          });
        } else {
    ...
    manipulateOptions: {
      ...
      value: function manipulateOptions(options) {
        ...
        const cssModulesOptions = {
          ...pluginOptions.defaultOptions.cssModulesOptions,
          ...currentConfig.cssModulesOptions,
        };
    ...
    visitor: {
      ...
      value: {
        ImportDefaultSpecifier: function(path, p) {
          ...
          const {
            variableDeclarator, variableDeclaration, memberExpression, callExpression, stringLiteral, identifier,
          } = t;
          ...
          if (typeof value == "string" && pluginOptions.matchExtensions.test(value)) {
            ...
            const newValue = relative(
              dirname(currentFile), resolve(dirname(currentFile), value),
            ).replace(".css", "_css.js");
        ...
        CallExpression(path, { file }) {
          ...
          const { normalisePath, requireCssFile } = pluginOptions;
          const {
            isStringLiteral, isExpressionStatement, memberExpression, callExpression, stringLiteral, identifier,
          } = t;
          ...
          if (pluginOptions.matchExtensions.test(value)) {
            ...
            const newValue = relative(dirname(currentFile), resolve(dirname(currentFile), value))
              .replace(".css", "_css.js");
